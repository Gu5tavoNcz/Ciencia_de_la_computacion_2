...
void leerText(vector<int>&arreglo, char *text)
{
    ifstream fuente;
    fuente.open(text,ios::in);
    if(fuente.is_open()){
        char cad=fuente.get();
        while(!fuente.eof())
        {
            arreglo.push_back((int)cad);
            cad=fuente.get();
        }
        fuente.close();
    }
...
void escribirText(vector<char>arreglo, char *text)
{
    ofstream salida;
    salida.open(text,ios::out);
    if(salida.is_open())
    {
...
vector<char> encriptar(vector<int>arreglo,int n)
{
    int n_encr;
    char caracter;
    vector<char>cadena;
    ...
    {
        n_encr=arreglo[i]*n;
        caracter=(char)suma_mod(n_encr,0,256);
        cadena.push_back(caracter);
    }
    return cadena;
}

vector<char> desencriptar(vector<int>arreglo,int llave)
{
    int n_des;
    char caracter;
    vector<char>cadena;
    for(int i=0;i<arreglo.size();i++)
    {
        n_des=arreglo[i]*llave;
        caracter=(char)suma_mod(n_des,0,256);
        cadena.push_back(caracter);
    }
    return cadena;
}
...
void ejercicio_4()
{
    vector<int>arreglo_1;
    vector<int>arreglo_2;
    vector<int>arreglo_3;
    int num_random,inverse;
    do{
        num_random=rand()%257;
        inverse=inversa_mod(num_random,256);
    }while(inverse==0);

    leerText(arreglo_1,"ficheroTexto.txt");
    cout<<"Tu numero de encriptacion es: "<<num_random<<"\nTu clave para desencriptar es: "<<inverse<<endl;

    vector<char>cadenaEncriptada=encriptar(arreglo_1,num_random);
    escribirText(cadenaEncriptada,"encriptado.txt");
    leerText(arreglo_2,"encriptado.txt");

    vector<char>cadenaDesencriptada=desencriptar(arreglo_2,inverse);
    escribirText(cadenaDesencriptada,"desencriptado.txt");
    leerText(arreglo_3,"desencriptado.txt");
